...
const GetInTouchForm = () => {
  ...
  const serviceOptions = ['Full-service Management', 'DIY Equipment Hire'];

  // Sanitize input to remove invalid characters and limit length
  ...
  const validateForm = () => {
    ...
    if (!formData.services.length && !formData.serviceOption) {
      return 'Please select at least one service or service option.';
    }
    return null;
  };
  ...
  const handleSubmit = (e) => {
    ...
    // Sanitize form data for EmailJS
    const sanitizedFormData = {
      name: sanitizeString(formData.name),
      email: sanitizeString(formData.email),
      phone: sanitizeString(formData.phone) || 'Not provided',
      message: sanitizeString(formData.message),
      services: formData.services.length ? formData.services.join(', ') : 'None selected',
      equipment: formData.equipment.length ? formData.equipment.join(', ') : 'None selected',
      service_option: sanitizeString(formData.serviceOption) || 'Not selected',
    };
              ...
              <>
                ...
                <form ref={formRef} onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <motion.div
                    ...
                    className="relative"
                  >
                    <label className="text-gray-300 text-sm font-medium mb-2 block" htmlFor="services">
                      Select Services
                    </label>
                  ...
                  <motion.div
                    ...
                    className="relative"
                  >
                    <label className="text-gray-300 text-sm font-medium mb-2 block" htmlFor="equipment">
                  ...
                  <motion.div
                    ...
                    className="relative"
                  >
                    <label className="text-gray-300 text-sm font-medium mb-2 block" htmlFor="service_option">
                      Service Option
                    </label>
                    <select
                      name="serviceOption" // Fixed: Changed from service_option to serviceOption
                      id="service_option"
